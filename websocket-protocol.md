...
| `auth.status` | — | `{ authenticated: bool, session? }` |
...
- **Returns**: a `RecommendationSet` — `{ picks: [Recommendation], not_enough_memory: [Recommendation], note? }`. A `Recommendation` is `{ model_id, display_name, role, rationale, download_mb, already_installed, fit: "fits"|"too_big"|"server_provided"|"unknown", acceleration, is_local, requires_cloud_consent, trust_tier, score }`. `picks` is ranked best-first; `note` is always present when `picks` is empty (machine too small, no cloud key, nothing in the lane) and may flag a cloud top pick.
- Ranks registry models for this machine + intent. Pure over the registry + detected hardware.

#### `models.setup_plan`
- **Params**: same as `models.recommend`.
- **Returns**: `{ machine, recommended?: Recommendation, alternatives: [Recommendation], needs_more_memory: [Recommendation], note? }` — a host-actionable onboarding plan. `machine` is a plain-language hardware summary.
...
- **Pushed** to subscribed UI clients while a `models.pull` / `models.install` is downloading. **Params**: `{ model, event }` where `event` is a tagged `DownloadEvent`: `{ event: "started", model, total_files, total_mb }`, `{ event: "file_started", filename, index, total_files, size_mb }`, `{ event: "file_completed", filename }`, `{ event: "completed", model }`, or `{ event: "failed", error }`. File-level granularity (a few dozen events per pull). Subscribe via the standard UI-event subscription.
...
#### `models.install`
- **Params**: `{ id?: string, name?: string, model?: string }`
- **Returns**: `{ path: string }`
...
#### `evolution.run`
- **Params**: `{ policy?: { pressure_threshold?: number, budget?: number }, dry_run?: boolean, harness_baseline_metrics?: HarnessMetrics, harness_candidate_metrics?: HarnessMetrics }`
- **Returns**: `{ plan: EvolutionPlan, steps: [{ component, ran, applied, outcome }], evolved: [..], pending_approvals?: [{ fingerprint, mutation, component, safety_affecting, rationale, reason }] }`
...
- **Returns**: an ack `{ accepted: true, session_id: string }`. The reply does **not** carry the answer — for attached supervised agents, the daemon reverse-calls the agent's `agent.chat` handler and fans the streamed reply back as `agents.chat.event` notifications (`kind: "token" | "tool_call" | "approval_pending" | "goal_evaluated" | "done" | "error"`) keyed by `session_id`. For in-daemon declarative agents, no child process or reverse call is needed: the daemon runs the declarative runner directly and emits the same `agents.chat.event` stream. `stream: false` still returns via the same event channel as a single terminal frame.
...
- `declagents.list` — **Params** `{}`. **Returns** `{ agents: [{ id, name, kind:"declarative", enabled, tools, goal?, scenarios }] }`.
