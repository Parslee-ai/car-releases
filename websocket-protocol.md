...
The dispatcher follows standard JSON-RPC classification: an `unknown method: ...` response has code `-32601`; request parameter extraction/deserialization failures have code `-32602`; an actual handler failure remains `-32603`. Every `-32602` message retains the missing or malformed field detail (for example, `invalid params: key is required`) rather than collapsing to opaque internal-error text. Callers must use the numeric code—not English text—to distinguish reachability from a malformed request or an internal failure.

#### `capabilities.list`

Runtime discovery for clients that cannot read files or run `car capabilities`.

- **Params:** `{}`.
- **Returns:** `{ caller_role, count, methods: [{ method, role }] }`, sorted by method through the generated capability table. Each role is one of `agent`, `owner`, `operator`, or `host` and is the same derived role committed in `docs/capabilities.json`.
- **Filtering:** an agent-scoped session receives `agent`, resource-conditional `owner`, and ungated `operator` rows, but never `host`; an unbound daemon session receives `operator`; a host-management session receives `host`, `owner`, and `operator`. Ownership remains checked against the concrete resource when an owner method is called.

The package form is `daemonCall("capabilities.list", "{}")` / `daemon_call("capabilities.list", "{}")`: the second argument is a JSON string even though the wire params are an object. The generated typed forms are `capabilitiesList("{}")` and `capabilities_list("{}")`.
...
- `feedback.v1` is optional and gates every `feedback.*` method: a negotiated v3 session that did not request it is refused with `-32008` (`protocol capability mismatch:`) naming the capability. The bundled host requests it as an optional capability so an older daemon still connects.
...
**`initialize` negotiates the protocol version.** The `initialize` result carries back the `protocolVersion` the client asked for when it is one this server supports (`car_mcp::SUPPORTED_VERSIONS` — `2024-11-05` and `2025-06-18` today). Absent, non-string, and unsupported all get `car_mcp::PROTOCOL_VERSION` (`2025-06-18`, the newest entry) instead, as a normal result rather than a JSON-RPC error, leaving the client to proceed or disconnect. The append-plus-move design is what made the 2025-06-18 bump safe: a client still asking for `2024-11-05` keeps getting it back rather than being handed a revision it never asked for. `2025-03-26` is deliberately absent — that revision makes receiving JSON-RPC batches a MUST, which this server does not implement; `2025-06-18` removed batching again.

**`outputSchema` / `structuredContent` (2025-06-18).** Tools whose result is a JSON object declare an `outputSchema` in `tools/list`, and their `tools/call` results carry `structuredContent` — the parse of the same serialization the text block already holds, so a `2024-11-05` client reads identical data from `content[0].text`. Array- and prose-valued tools (`memory_query`, `skill_find`, `skill_list`, `memory_add_fact`, `skill_ingest`) declare no `outputSchema` and never emit `structuredContent`, because the spec requires it to be an object and wrapping would desynchronize the two representations. Elicitation is not declared (it is a client capability; CAR-as-server never initiates one) and `resources/templates/list` stays unimplemented by decision — every URI CAR exposes is fully enumerable through `resources/list`.
...
|-------------------------------|--------|
| absent | **allowed** — requiring the header would break every client that predates it, and the 2025-06-18 delta on this surface is additive result keys an older client ignores, so dispatch answers a silent client correctly whichever revision it negotiated |
...
#### Daemon-only tool groups: eventlog, workflow, scheduler (car#972 Track C)

The production daemon registers the six read tools by default. Calls to this
daemon tool group require
`Authorization: Bearer <generic-daemon-token>` on each HTTP request; tenant
binding uses `X-Car-Tenant-Id`. Supervised-agent credentials are unsupported:
requests carrying `X-Car-Agent-Id` are explicitly refused before dispatch, so
they cannot replace a live agent connection or silently lose its identity.
Agents must use their existing WebSocket connection. The bridge
passes these credentials through `session.auth` and protocol negotiation on a
fresh daemon session for each invocation. Missing or invalid credentials fail;
no daemon, host, environment, or disk credential is substituted. Host-management
elevation is not supported through this bridge. Requests never share an
authenticated connection. Offline stdio still omits these tools. Existing assistant and other MCP tools
retain their existing authentication behavior.


Three more groups register through the same per-`Server` seam as the assistant trio (`car_mcp::register_daemon_tools`, given a `car_mcp::DaemonClient` that carries the caller's session authority). Each tool is a **thin adapter over an existing daemon JSON-RPC method** — arguments forwarded verbatim as the method's params, the method's result serialized verbatim as the tool text (plus `structuredContent` where the result is a JSON object) — so the daemon handler stays the single implementation and the single authority check: a bound agent token sees exactly what the same method would give it over WS. On stdio none of these exist (`-32601`), by design.

| Tool | Daemon method | Notes |
|---|---|---|
| `events_query` | `events.query` | audit-trail query; same `EventQuery` params |
| `events_stats` | `events.stats` | event/span counts |
| `events_cost_by_agent` | `events.cost_by_agent` | per-agent token/cost receipts; array result, text only |
| `workflow_verify` | `workflow.verify` | pure static manifest check |
| `workflow_list_paused` | `workflow.list_paused` | resumable approval gates; array result, text only |
| `scheduler_list` | `tasks.list` | scheduled command tasks; array result, text only |
| `workflow_run` † | `workflow.run` | runs real stages — `openWorldHint: true`, `destructiveHint: true` |
| `workflow_resume` † | `workflow.resume` | continues side-effecting stages |
| `scheduler_schedule` † | `tasks.schedule` | can install a durable OS schedule (launchd/cron/schtasks) |
| `scheduler_unschedule` † | `tasks.unschedule` | removes the OS schedule and the stored task |

† **Mutating tools are opt-in.** The read surfaces register whenever the embedder supplies a daemon client; the four mutating/triggering tools register only when the embedder sets `DaemonToolOptions { enable_mutations: true }`, because an MCP host may auto-approve unattended and a workflow stage or scheduled program can do anything the runtime can. When not enabled they are absent from `tools/list` and a call is `-32601` — absent, not advertised-but-broken.

**External-agent invocation is deliberately excluded** from this surface. The fourth §6 group is blocked on the §7 ancestry gap: the recursion guard decides from process environment (`$CAR_INVOKED_BY` via `ancestry()`), while the daemon path only records a per-call caller — exposing invocation over MCP before that gap closes would let a nested agent bypass the guard. It stays off this surface until §7 lands.
...
A callback that knows retry cannot recover may opt into terminal failure:

```json
{"jsonrpc": "2.0", "id": "cb-1", "error": {"code": -32000, "message": "receipt journal is unavailable", "data": {"terminal": true}}}
```

`error.data.terminal` is optional and defaults to `false`. The daemon never
infers it from `error.message`. When true, the engine stops retries, aborts and
rolls back the current proposal, and marks the failed `ActionResult` with
`terminal: true`; the daemon then halts this WebSocket session so later
`proposal.submit` calls return rejected action results without dispatching a
tool. The halt is connection-local and is discarded on reconnect. A host-role
connection may clear its own latch with `session.clear_halt`; durable run
failure remains an explicit `runs.complete` responsibility.

The NAPI package exposes `TerminalToolError`, and Python exposes
`car_runtime.TerminalToolError`. Throw or raise that distinguished class from a
tool handler to produce this typed wire bit. Ordinary exceptions and legacy
JSON results preserve their previous behavior.

The Rust client at `car_ffi_common::proxy::DaemonClient` exposes `register_handler(method, handler)` for legacy `Result<Value, String>` callbacks. Typed tool bindings use `register_typed_handler(method, handler)` and return `ServerRequestFailure::terminal(message)` only after recognizing their distinguished exception; both install a closure that the receive loop dispatches inbound `tools.execute` requests to.
...
- **Returns**: `{ ok: true, auth_enabled: bool, agent_id?: string, method_allowlist?: string[], memory_namespace?: string, parslee?: ParsleeIdentity }`,
...
- **Lifecycle-agent identity binding (#169)**: when `agent_id` is
  ...
  set on spawn (#172). When that registration carries `method_allowlist`, the
  successful response echoes the canonical list and the daemon checks every
  later request or notification before any method handler runs. `session.auth`
  and `server.handshake` are implicit transport-establishment calls. A method
  outside the list returns `-32601` with the named
  `agent_method_not_allowed` error; an out-of-scope notification is dropped.
  The scope only removes access — it never grants host, owner, or other
  authority that the method's normal handler requires. An absent allowlist
  preserves the historical unrestricted token; an empty list permits only the
  two implicit transport-establishment calls.
...
The `feedback.*` namespace is CAR's in-app feedback intake (Parslee-ai/car#1137):
compose a redacted diagnostic bundle, preview it, durably spool it, and read the
local outbox. **All four methods first require the connection to negotiate the
optional `feedback.v1` capability in `server.handshake`**; protocol version
alone is not consent to this surface. They also require the exact current
protocol version (`car-proto::PROTOCOL_VERSION`) and return `-32005` (`protocol
handshake required:`) until that handshake succeeds, because a legacy host has
no consent UI for feedback. They are *not* host-management-gated: any
authenticated client that negotiated the current protocol and `feedback.v1`
may call them. Failures (spool I/O, invalid params, unknown ids, the throttle)
surface as ordinary JSON-RPC errors whose message names the cause.

**Closed world by construction**: no method on this surface accepts a path
parameter, and unknown params are ignored — a client-sent `path` or
`log_tails` field is inert. The log set is built daemon-side: the supervised
agents' `<id>.{stdout,stderr}` log files (ids read from the `agents.json`
registry manifest — only the `id` field; env maps never enter the bundle) plus
the daemon stderr-tee file (`<CAR_HOME>/logs/car-server.stderr.log`). A client
cannot name a file to include.

**Consent semantics (toggles are authoritative)**:

- `include_diagnostics: false` (the default) collects **no** log tails, no
  doctor report, and no runtime context — the bundle carries an explicit
  `diagnostics` manifest item noting the exclusion.
- `include_screenshot: false` (the default) drops any supplied
  `screenshot_b64` bytes entirely: no screenshot item exists anywhere in the
  bundle, not even a manifest note. With the toggle on, the host-captured
  JPEG (base64, ≤5MB decoded — the ChatAttachments per-image cap) rides
  `bundle.runtime_context.screenshot` as `{jpeg_b64, byte_len}` and is
  accounted as a `screenshot` manifest item.

A current-protocol session without `feedback.v1` gets the standard `protocol capability mismatch:` refusal and nothing is composed or spooled.

#### `feedback.compose_preview`
- **Params**: `{ description: string, include_screenshot?: boolean = false, include_diagnostics?: boolean = false, screenshot_b64?: string }`
- **Returns**: `{ preview_id: string, bundle: RedactedBundle, manifest }` — `manifest` is the bundle's own manifest echoed top-level for cheap rendering; `preview_id` is an opaque handle to the exact composed bytes
- The consent preview: assembles the exact post-redaction bundle `feedback.submit` would spool — both run the one compose path — and **persists nothing to the spool**. The composed bundle is held in a small in-memory daemon cache (5 entries per CAR home, 10-minute TTL) keyed by `preview_id` and the composing WebSocket session, so another session cannot redeem a leaked handle. `feedback.submit { preview_id }` from the owning session enqueues the PREVIEWED bytes byte-for-byte (the same-bytes consent handoff). A supplied screenshot that fails validation (base64 decode, JPEG SOI/EOI magic, ≤5MB decoded) never aborts: it soft-fails to an omitted manifest item with a why-note. A `feedback.submit` carrying an invalid/expired/foreign-session `preview_id` returns a `PREVIEW_EXPIRED` error instead of silently composing unseen bytes — the host re-previews and asks the user to submit again; only a submit WITHOUT a `preview_id` composes fresh (explicit consent to current state).

#### `feedback.submit`
- **Params**: the `feedback.compose_preview` params **plus** `{ lane: "authenticated" | "anonymous", org_id?: string, preview_id?: string }` — with a valid `preview_id` the daemon enqueues the stored previewed bundle byte-for-byte; an expired/unknown `preview_id` is a `PREVIEW_EXPIRED` error (re-preview and resubmit); omitting `preview_id` composes fresh. For the authenticated lane, `org_id` is optional compatibility input and never authority: the daemon derives the organization from its signed-in Parslee session and rejects a supplied mismatch.
- **Returns**: `{ submission_id: string, client_submission_id: string, state: { state: "queued" }, source: "local" }`
- Composes the same bundle and durably enqueues it in the local spool (`<CAR_HOME>/feedback-outbox`), then wakes the upload drain. The identity lane is bound at enqueue — the consent moment — into durable spool state with no rewrite API, so a signed-out submission can never later drain with a bearer: `lane: "authenticated"` requires a signed-in Parslee session with an active organization; that session-derived id is persisted, and any client-supplied `org_id` must match it. `org_id` alongside `lane: "anonymous"` is ignored. `client_submission_id` is the immutable idempotency key. A client-side per-install throttle (10 submissions per rolling hour, checked before enqueue) rejects the call with a message naming `retry in <n>s`.

#### `feedback.status`
- **Params**: `{ submission_id?: string, limit?: integer }`
- **Returns**: `{ entries: [SpoolEntryRow], has_more: boolean, staleness: { pending_count, oldest_age_days } | null }` — filtered to the one entry when `submission_id` is given (unknown id → error; `limit` ignored), otherwise the same bounded rows as `feedback.list`. If the selected bundle is corrupt, its durable summary still returns with `description: null` instead of failing the RPC.

#### `feedback.list`
- **Params**: `{ limit?: integer }`
- **Returns**: `{ entries: [SpoolEntryRow], has_more: boolean, staleness }` — the newest `limit` merged local/server rows, kept in oldest-first order. `limit` defaults to 100 and is clamped to the hard maximum of 200; `has_more` says whether older rows were omitted.

A `SpoolEntryRow` is
`{ id, client_submission_id, state, lane, title, created_at, age_days, source: "local" | "server", note? }`:
`state` is the tagged spool lifecycle object — `{ state: "queued" }`,
`{ state: "sending" }`, `{ state: "acknowledged", server_id }`,
`{ state: "terminal_actionable", reason: "auth_required" | "reconsent_required" | "forbidden" }`
(waits for the user; never retried automatically), or
`{ state: "terminal_rejected", message }` (server 400-class; never retried).
`lane` is `{ lane: "anonymous" }` or `{ lane: "authenticated", org_id }`.
`created_at` is unix seconds at enqueue; `age_days` is whole days since, at
list time. `source` distinguishes local spool rows from server rows: for the
authenticated lane, `feedback.list` merges the signed-in user's rows from
CAR's own Parslee feedback intake (`GET /api/v1/orgs/{orgId}/car-feedback/mine`,
fetched via the Parslee session; any fetch failure degrades to
local-only, never an error) by acknowledged server id — a local row whose
server counterpart advanced shows the server state. Queued entries drain
to `POST /api/v1/orgs/{orgId}/car-feedback` only while the anonymous capability probe
(`GET /api/v1/car-feedback/capability`) advertises the entry's lane:
`authenticatedIntake` gates signed-in entries, and anonymous entries hold
while `anonymousIntake` is `false` (the v1 server reality). `note?` carries per-row daemon annotations on TERMINAL settles (the
drain persists upload-time omission notes when an entry settles
terminal); for successful uploads, omissions surface at submit time in
the manifest/UI warning, not as a persisted row note.
`staleness` is non-null when any pending entry has waited more than 2 days.
...
- **Notes**:
  ...
  - On macOS, `total` (and `unread` for nested mailboxes) is read from Mail's
    read-only local **Envelope Index** rather than live Apple Events, so both
    can lag Mail's live state by a sync cycle. When that index is unreadable
    (no Full Disk Access, fresh profile) the listing falls back to per-mailbox
    Apple Events — slower but live; a mailbox the index has no row for keeps a
    best-effort `total: 0` and its observed unread rather than failing the
    listing.
...
- **Notes**:
  ...
  - Rows come back newest first, and that ordering is **global, not per
    ...
    **before** `limit`, so a narrow window still returns the newest `limit`
    matches.
  - On macOS, ordering uses an ID/date snapshot of the mailbox (or supported
    filed-mailbox date window), then reads details for at most `limit`
    candidates per account. This preserves newest-first results regardless of
    Mail's collection order without reading every message's details. An
    unreadable ordering snapshot returns `available: false`, never an
    arbitrary prefix reported as newest mail.
...
- **Notes**:
  - `chat_ids` contains the `chat.guid` values returned by `messages.chats`.
    ...
    treated as opaque. `handle` is commonly absent on rows where `is_from_me`
    is `true`.
  - Rows are newest first. `since` is applied in SQLite **before** `limit`, and
    `date` is RFC3339 converted from Messages.app's Apple-epoch nanoseconds.
  - `text` is decoded from `attributedBody` (falling back to the text column)
    only when `include_body` is true, and is bounded to 100,000 characters.
  - If Full Disk Access or the database is unavailable, the method returns
...
- **Fit fields** (car#1399): every row says whether it fits the machine that answered, so a first run is not offered thirty models it cannot hold. `fit` is `"fits" | "too_big" | "unknown"`, judged by the same rule `models.recommend` partitions `picks` from `not_enough_memory` on — the cold-load estimate at the recommendation context (weights + KV cache + runtime overhead + transient margin) against the daemon's **active** local-model resource policy ceiling (the one `models.preflight` admits against; `models.resource_policy.set` updates both it and the persisted file in one call). `estimated_peak_mb` is that estimate in MB, `null` when the row's memory is not this machine's, the OS owns the implementation, or it declares neither a size nor a RAM figure. OS-provided rows (`WindowsSpeech`, Apple FoundationModels) report `fit: "fits"` even with no declared size because their memory is outside CAR's model budget. `platform_compatible` is `false` for a Metal-only row (MLX, managed vLLM-MLX, Apple FoundationModels) on a non-Apple machine, for WindowsSpeech or a `windows-only` tagged row off Windows, and for a `linux-only` tagged row off Linux — the rows the recommender never offers there. **Rule for non-local rows**: remote API, delegated and operator-managed external rows report `fit: "fits"`, `estimated_peak_mb: null`, `platform_compatible: true`, because their memory and accelerator are the server's, not this machine's. `unknown` is never a reason to hide a row, and it is also what an older daemon's rows read as — clients must fail open on visibility. The annotation is computed per call from `HardwareInfo::detect()` and the active policy and is never persisted: no receipt, tombstone, `models.json` row or policy file gains these fields. The list itself is **not** filtered — every registered row is still returned in registry order with every pre-existing field unchanged; hiding is the client's job (`car models list` hides `too_big`, platform-incompatible and deprecated rows unless `--all`; the macOS Model Library keeps them behind a "Show N models that don't fit this Mac" disclosure and the Settings picker always keeps the saved selection visible).
- **Catalog identity fields**: `deprecated` is `ModelSchema::deprecated` — superseded in the catalog; the row stays listed (an installed deprecated model still works) and clients lead with current models by sorting or hiding on it. `family` (model line, e.g. `qwen3`) and `version` (label within the family) are published **for local rows only**; remote rows report `null` for both because this response deliberately carries no upstream identifier for the managed `parslee/openrouter/<alias>` rows, whose `family` and `version` name the upstream model — `models.search` is the view that publishes them for every row. All six fields are additive: a client parsing a response from an older daemon sees them absent and defaults to `unknown` / `null` / `true` / `false` / `null` / `null` rather than failing the whole catalog.
...
- Searchable registry view for host UIs and non-technical installers. Each entry is a `models.list_unified` row — **every** field of it, including the additive fit annotation (`fit`, `estimated_peak_mb`, `platform_compatible`) and `deprecated`, with exactly the values that view publishes for the same row — plus `{ family, version, tags, pullable, upgrade? }`. The flattened row keeps `models.list_unified`'s snake_case names (`estimated_peak_mb`, `platform_compatible`); only search's own fields are camelCase, and those are single words. `family` and `version` are populated for **every** entry here — including the managed `parslee/openrouter/<alias>` rows and every other remote row for which `models.list_unified` publishes `null` — because this is the view that names an alias's upstream family. The exact key set of both views is pinned by `car-server-core/tests/models_surface_gate.rs`, so a field added to the unified row reaches search as a reviewed change, never an incidental one. `pullable` is true for local/Hugging Face-backed models CAR can install itself. `upgrade` is present when an installed model has a curated replacement.
...
- Ranks registry models for this machine + intent using the running engine's active local-model resource policy — the same policy used by `models.preflight` and list/search fit annotations. The engine initializes it from persisted state (missing state defaults to Everyday), and `models.resource_policy.set` saves and applies it together. Everyday Assistant/Balanced puts known-fitting, tool-capable local candidates inside the conservative recommendation target first; heavier and unknown-memory rows remain visible. Policy loading does not read credentials.
...
- **Returns**: serialized `ProposalResult`; each `ActionResult` may carry `rolled_back: true` when the action succeeded but an enclosing abort restored the proposal's state transaction. False is omitted. `status` still reports whether the action executed, `state_changes` is empty after rollback, and `error` may retain a human warning that external effects remain; never match that warning to detect rollback.
- Triggers `tools.execute` callbacks to the client for each `tool_call` action. See `docs/agent-ir-spec.md` for the proposal and result shapes.
...
- After a `tools.execute` callback explicitly returns terminal failure, this WebSocket session is halted. Later submissions return the normal whole-proposal `rejected` result shape and dispatch no tools until a host clears the latch or the client reconnects. No run is synthesized or completed automatically.
...
- `scheduler.os_install` — **Params**: same as `os_render`. **Returns**: `{ label, backend: "launchd"|"cron"|"schtasks", detail, conflict_preserved? }`. Writes the plist to `~/Library/LaunchAgents` and `launchctl load`s it (macOS), appends a tagged crontab line (Linux), or registers a Windows Task Scheduler task via `schtasks /Create /XML` (Windows). Idempotent — replaces any prior schedule for the same task. On macOS the replacement is divergence-safe: the prior plist is claimed by rename (same inode) and the new plist published atomically; if some process wrote through a file descriptor it opened before the claim, those bytes are preserved at a sibling `<plist>.car-conflict-<utc>` file — never deleted or overwritten — and `conflict_preserved` carries that path (absent otherwise, and always absent for cron/schtasks). Also **persists the task to `~/.car/tasks/`** so the store stays the authoritative set of scheduled tasks (which `os_reconcile` reaps against); a persist failure aborts the install.
...
- `tasks.schedule` — **Params**: `{ name, program, args?: string[], cadence: { interval_secs?: number | cron?: string }, durable?: boolean, working_dir?, env?, timeout_secs? }` — the cadence is expressed once (exactly one of `interval_secs` / `cron`). **Returns**: `{ id, durable, backend: "launchd"|"cron"|"schtasks"|"daemon", task }` (`schtasks` = Windows Task Scheduler). CAR picks the backend: `durable: true` (default) installs to the OS scheduler so it fires even when car-server is down — **`*/N` cron is normalized** (expanded to a launchd calendar array) so the same input works on macOS and Linux; `durable: false` uses the in-daemon timer (fires only while car-server runs, `backend: "daemon"`), which supports `interval_secs` only (a cron cadence requires the OS backend). The task is persisted to `~/.car/tasks/` first (authoritative for `os_reconcile`). Commands execute with the daemon OS user authority; no per-task permission tier is enforced.
...
- The exact daemon-owned OAuth slot (`service: "car"`, `key: "OPENROUTER_OAUTH_API_KEY"`) is reserved on the generic surface: `secret.put`, `secret.get`, and `secret.delete` fail with `reserved_private_secret`. Use `openrouter.auth_start` / `openrouter.disconnect`; status/list remain metadata-only and never return its value. The authenticated local-operator broker described below is an internal exception so the CLI's own provider resolver can load this credential.
...
#### `secret.get`
- **Params**: `{ service?: string, key: string, operator_broker?: boolean }`. `operator_broker` is reserved for CAR's local CLI client; external callers should omit it.
- **Returns**: `{ value: string }`
- A connection authenticated as a supervised agent is the credential-read broker path. The daemon derives `agent_id` from `session.auth` (never from params), constructs a `secret.get` tool action carrying the canonical service/key and that identity, and evaluates it through the connection's policy engine before reading. A denial returns an error whose JSON message has `code: "policy_denied"`; clients must not fall back to direct storage after any daemon response.
- The native CLI uses `operator_broker: true` only for a loopback URL and authenticates with the owner-only local host-token file. The ordinary daemon token is available to loopback dashboard clients and therefore is not sufficient proof of the daemon user's filesystem identity; the cross-host `CAR_AUTH_TOKEN` override is never used for this role. The daemon accepts operator mode only from an authenticated host session with no supervised-agent identity and only while daemon auth is enabled. This allows `car keys`, provider resolution, and Parslee auth to reuse the daemon's already-approved credential-store identity, including their private provider/auth slots. A supervised agent cannot set this flag to bypass policy, and a `--no-auth` daemon cannot grant it.
- Every broker attempt appends one redacted event. Supervised events are `action_succeeded` with `outcome: "read"`, `action_failed` with `outcome: "secret_store_error"`, or `policy_violation` with `outcome: "policy_denied"`; their data contains `tool`, `service`, `key`, authenticated `agent_id`, and outcome (plus denying policy names). Operator events contain `tool`, `service`, `key`, `caller_role: "operator"`, and outcome. Values and backend error text are never journaled.
- Environment-keyed resolution still checks a non-empty process environment value before this RPC. Every daemon refusal is terminal. A supervised process also fails closed on transport failure; the operator CLI falls back to its existing direct store only when no daemon response arrives within the two-second broker bound. This keeps standalone CLI use working while preventing an answered denial from becoming a local read.
- Host and ordinary authenticated callers that omit `operator_broker` retain the existing generic `secret.get` behavior. Daemon-private slots remain unavailable on that generic path.
...
#### `session.clear_halt`
- **Params**: `{}`
- **Returns**: `{ cleared: boolean, halted: false }`
- **Packaged clients**: `HostClient.sessionClearHalt` / `session_clear_halt` uses the runtime's existing connection and never loads a host token or authenticates a new connection. It fails unless the caller has already explicitly authenticated that same connection as host. It cannot clear another runtime's latch; ordinary clients recover by reconnecting.
- Clears the terminal-tool-failure latch on this exact WebSocket connection. The caller must have authenticated this connection with `session.auth { host_token }`; ordinary authenticated clients are rejected. `cleared` is false when the latch was already clear. Reconnect is the ordinary client recovery path and starts with a fresh, clear latch; this method does not alter or synthesize a durable run outcome.
...
- Every model that AUTHORED part of a change is recorded on the session
  ...
  reach: it checks a resolved pin, but adaptive routing can choose a different
  model on each request. A set rather than one name, because a turn terminal is
  journaled per iteration and an unpinned session may route each iteration
  differently; a reviewer judges the accumulated diff, not the last iteration.
  Models that served turns INSIDE an iteration another model finished are
  counted too (car#1333) — the terminal names only whoever reached it, so a
  model could otherwise write most of a change and still sit on the panel
  judging it.

  When neither pin source below supplies a coder model, self-heal also prevents
  the collision before inference: every turn carries the canonical review-seat
  names in `IntentHint.exclude_models` with `strict_exclusions: true`. If no
  independent model remains, generation stops before provider dispatch with
  `failure_kind: "configuration"`; the cadence releases the item claim without
  item backoff or a public failure comment, because the remedy is in
  `heal.toml`, not the backlog item. The durable `authored_by` check remains the
  delivery backstop for routing changes and already-running cadence state.
...
- A pinned coder that resolves to the same model as a review seat is REFUSED at
  ...
  **Two pin sources.** `coder.start` resolves a session's model as the request
  ...
  is the coder chosen by the router; in that case every review seat is a strict
  routing exclusion, with the `authored_by` gate above retained as a delivery
  backstop.
...
- `coder_pin` is the model the coder will run on and which file pinned it —
  ...
  **Absent means unpinned, which is a third state.** Neither file names a
  model, so adaptive routing picks per request while strictly excluding every
  review seat. Nothing before the run can say which remaining model it will
  pick. That is different from "pinned to something the panel rejects", and
...
- **Returns**: `{ outcomes: [[repo, outcome]], skipped_overlap }`, where each
  `outcome` is tagged. An opened result is
  `{"outcome":"opened", repo, number, pr_url, gate, ci, delivery}`. `ci` is
  `{head_sha, state, checks:[{name,state}], observation_error?}`; every state is `"green"`,
  `"pending"`, or `"red"`, and it combines GitHub check runs with legacy
  commit-status contexts for that exact delivered SHA. An empty rollup is
  pending rather than green. A failed CI lookup preserves the opened result,
  with pending state, no checks, and an `observation_error` diagnostic; it does
  not retry publication or release its held claim. `delivery` is the matching plain summary, such as
  `delivered with green checks and ready for review at <sha>` or
  `delivered red on windows at <sha>`. The other variants remain
...
- This can run a real coder session and open a real pull request. After
  reconciliation, the host reads CI for the delivered head and includes it in
  the report; a head race is refused rather than reporting another commit's
  checks. It goes through the same service the cadence uses, so it shares the
  single-sweep lock and the claim ledger. It does not merge the pull request.
...
- **Params**: `AgentSpec` — `{ id, name, command, args?, cwd?, env?, restart?, max_restarts?, backoff_secs?, auto_start?, interpreter?, method_allowlist?: string[] }`. `restart` is `never | on_failure | always`. `id` must be filename-safe (alphanumeric + `-_.`).
...
- **`method_allowlist`** scopes the per-agent token to a subset of daemon methods whose generated capability role is `agent`, `operator`, or `owner`. Unknown and `host`-role names are rejected at upsert/install. The list narrows existing authorization rather than granting the listed role. Omit it for backward-compatible unrestricted behavior; pass `[]` to deny every application method. When an upsert omits both the token and the allowlist for an existing scoped agent, both are retained together so an older client cannot silently widen the token. Removing the scope requires supplying/rotating the token explicitly.
...
- **Params**: `AgentManifest` — the nested TOML shape from
  ...
  per-kind sub-table carries the rest. External-process transports accept the
  same optional `method_allowlist: string[]` scope as `agents.upsert`.
...
- **Params**: `{ id?: string, force?: boolean }`
  - `id` selects a single adapter (`"claude-code"`, `"codex"`, `"gemini"`, `"mini-swe-agent"`). Omit to check every detected adapter.
...
- **Ground truth**, not heuristic: delegates to each tool's own status command (`claude auth status` returning structured JSON, `codex login status` returning a single status line, etc.). Two adapters have no such command and say so in `reason`: Gemini (running it without args triggers a browser OAuth flow) falls back to credential-file shape inspection, and `mini-swe-agent` (which has no status command at all) reports `ready` when a provider API key **name** resolves from the daemon environment or from its own global `.env`. Neither is verified against the provider; a key that doesn't match the pinned model surfaces at invocation.
...
- **Params**: `{ id: string, task: string, stream?: boolean, session_id?: string, cwd?: string, allowed_tools?: [string], max_turns?: number, timeout_secs?: number, mcp_endpoint?: string, attachments?: [{ path: string, media_type?: string }] }`
  - `id` selects the adapter (`"claude-code"`, `"codex"`, `"gemini"`, `"mini-swe-agent"`).
  - `"mini-swe-agent"` is a **measurement baseline**, not a production engine — it is the control arm for `car coder-ab` (see `docs/coder-ab-guide.md`). It must be named explicitly: unnamed engine resolution (`coder.start` with `engine: "auto"` or a bare `"external"`) never selects it. It also has no default backbone of its own, so an invocation without `model` (and without `$MSWEA_MODEL_NAME` in the daemon environment) is refused up front rather than spawned; its model ids are **litellm** ids and need the provider prefix (`openai/gpt-5.5`, not `gpt-5.5`).
  - `task` is the prompt the agent runs to completion.
  - `stream` opts into per-event streaming over the existing `agents.chat.event` channel (default `false`). When `true`, the method returns an ack (`{accepted, session_id}`) and the runner fans `agents.chat.event` notifications to the originating host as the child emits stream-json events — `kind: "token"` per text content block, `kind: "tool_call"` per `tool_use` block, terminal `kind: "done"` with metadata folded into `finish_reason` (or `kind: "error"`). Every adapter emits events (car#213), but only two stream: `claude-code` forwards native stream-json events; `codex` synthesizes an Anthropic-Messages-shaped `Assistant` event per `item.completed` line (incremental `kind: "token"` / `"tool_call"` frames); `gemini` is text-only in 0.1.x (no event stream) and `mini-swe-agent` has none either (its trajectory JSON is read once after exit), so each emits one `Assistant` text event with the full reply on completion — a single-pop `kind: "token"` then the terminal `done`, not a token stream. `capabilities.streaming` (`agents.list_external`) says which is which. Each `tool_call` frame carries the `tool` name + `params` (the `tool_use` input) per the shape pinned in `docs/host-protocol.md` (plus a legacy `detail` = name alias).
  ...
  - `attachments` are images attached to the prompt, each `{ path, media_type? }` where `path` is an **absolute path on the daemon's filesystem** (external CLIs are local subprocesses, so images are passed by path, not base64). The runner hands each to the CLI in its native form: `claude-code` reads the bytes and inlines a base64 image block on stdin, `codex` passes each via `--image`, `gemini` references it with `@path` (staged into a temp dir first). The runner validates every path it reads/stages: files over 32 MB are skipped, and the `claude-code`/`gemini` paths verify the bytes are a real image by magic signature (PNG/JPEG/GIF/WebP) and derive `media_type` from the **content** — the supplied `media_type` is advisory, so a non-image file (e.g. a secret the path points at) is never inlined. Adapters advertise support via `capabilities.images` (`agents.list_external`); a CLI without image input ignores them (`mini-swe-agent` has none, and reports every supplied image in `dropped_attachments`). Unreadable / oversized / non-image paths are skipped.
...
- **Params**: `{ goal: string, repo?: string, adapter?: string, verify_command?: [string], union_verify_command?: [string], max_attempts?: number, distributed?: boolean, workers?: [string] }`
  - `adapter` selects the external coding CLI (`"claude-code"` default; `"codex"` / `"gemini"` as their adapters land). `"mini-swe-agent"` is not a Foreman adapter — it is the `coder-ab` measurement baseline and is excluded from unnamed engine resolution.
...
file, a missing `[coder]` table, or any missing key falls back to the documented
default; empty and zero values follow each key's semantics below. A malformed
file is logged and treated as absent (the daemon never fails to boot over it).
`CAR_CODER_CONFIG` overrides the path (for
...
- `engine_preference` — the order in which a *ready* external CLI is chosen for
  ...
  built-in `["claude-code", "codex", "gemini"]`. One detected adapter is never
  eligible here, in the list or through the outside-the-list fallback:
  `mini-swe-agent` is the `coder-ab` measurement baseline, so only an explicit
  `engine: "external:mini-swe-agent"` reaches it. Routing a real task to a
  control arm is choosing the control as the product.
...
- `max_session_wall_secs` / `max_agent_build_wall_secs` — end-to-end wall
  ...
  on-device generation in the inference worker is killed with its worker
  process, and a remote request is cancelled. The in-process fallback (worker
  disabled with `CAR_NO_INFERENCE_WORKER=1`, or unable to start) and
  FoundationModels cannot be interrupted mid-decode, so that generation
  finishes in the background: in-process MLX within
  `CAR_LOCAL_DECODE_TIMEOUT_SECS` (300 seconds by default) plus prefill;
  in-process Candle, off Apple Silicon, is bounded only by `max_tokens`; and
  FoundationModels whenever the framework call returns.
- `max_replay_events` (car#1311) — the per-live-session in-memory reconnect
  window. Default `2000`; `0` disables the cap. It is read when a session starts,
  so changing the file affects new sessions. This does not change sequence
  numbers or the durable event journal: trimming removes only the oldest events
  available for immediate WebSocket replay, and `coder.subscribe.events_skipped`
  names the resulting gap.
...
- **`distributed`** (car#1243, default `false`) farms this session's subtasks
  ...
  - **Where each subtask actually ran** is recorded (car#1322), as TWO records
    ...
    Runtime policies registered on the client session apply to this delivery
    gate exactly as they do to `foreman.run`. In particular, a
    `deny_tool = ["foreman.integrate"]` rule rejects every proposed patch before
    its build check and prevents it from reaching the delivery worktree. The
    gate writes `gate_accepted` / `gate_rejected` directly into that client
    session's runtime audit journal (car#1321); the delivery loop also projects
    those rows into the coder run's `<id>.events.jsonl` so the run remains
    independently reviewable after its originating connection is gone. Each
    invocation stamps a unique `gate_audit_scope` on its primary gate records;
    the coder projection accepts only that scope, so concurrent runs sharing a
    client session cannot copy each other's verdicts even with reused subtask IDs.

    **The journal records are not derived from the event.** Do not read the two
    as one thing with two spellings. Events on this stream include everything a
    supervised coding CLI writes to stdout, so a `foreman: "gate"` event is
    narration and a client should treat it as such; the journal rows originate
    in the merge gate using the session's shared runtime log, then the coder
    journal projection is written directly by the foreman loop. That is what
    makes them audit records for patches authored on machines this host does not
    control (car#1243).
...
- A session that is **not live but has a persisted snapshot** (the daemon
  ...
  `{ ok: true }`.
- A live session retains the newest **2,000 events** for reconnect replay by
  default. Operators can set `[coder] max_replay_events` in `~/.car/coder.toml`;
  `0` means unlimited, and the value is captured when a new session starts. When
  `from_seq` predates the retained window, the surviving events retain their
  original `seq` values and `events_skipped` reports exactly how many earlier events are
  unavailable. It is `0` for an in-window cursor and for a persisted non-live
  session, so replay loss is explicit rather than inferred from the response
  length.
...
- `declagents.invoke` — **Params** `{ id: string, input: string }`. **Returns** `{ output, turns, tool_calls, error?, goal? }`. Runs the agent on `input` entirely in-daemon (no process), bounded by a turn cap, with file tools rooted in an ephemeral scratch workspace. When the spec has a `goal`, CAR runs `goal.check` in that same scratch workspace after each pass, re-drives the agent with the verifier reason until the check exits 0, and returns `goal: { check, max_iterations, iterations, met, grounded, last_exit_code, last_reason }`; if the check never passes, `error` is `goal_not_met ...`. **Context policy.** With the spec's `context` at its default `"car"`, the run's growing history is compacted before each model call to fit the context window of the model that ran the turn (three quarters of it, measured on the provider's reported prompt size once there is one), pinning the identity and the original input and leaving the `[history compacted: ...]` system notice — the same compaction the assistant and coder loops use. With `context: "self"` CAR never compacts and the spec owns the transcript. The window is re-read after every call from the model that served it, so a mid-run fallback to a smaller model is compacted against the smaller window. A model absent from the catalog (or a catalog row whose `context_length` is `0`) has no known window. If the run never knew one, compaction is disabled for it and the daemon logs `compaction disabled: unknown context window for model <id>` once per invocation; if an earlier turn did know one, that budget is kept — losing it would leave the run unbounded over a routing detail — and the daemon logs `model <id> has no known context window; keeping the last known budget of N tokens` once instead. An unrecognized `context` value is not a parse error — it warns and falls back to `"car"`, so a hand-edited typo can never unregister the agent.
...
#### `infer`
- **Params**: `GenerateRequest { model?: string, model_id?: string, messages: Message[], system?: string, max_tokens?: number, temperature?: number, tools?: ToolDef[], context_query?: string, memory_intervention?: boolean | ProactiveMemoryRequest, response_format?: ResponseFormat, intent?: IntentHint, ... }`. `model_id` names an exact immutable catalog identity and is mutually exclusive with `model`.
- **Returns**: `{ text: string, tool_calls: ToolCall[], usage: { input_tokens, output_tokens }, model_used: string, trace_id: string, latency_ms: number, time_to_first_token_ms: number | null, stop_reason: string | null, local_last_resort?: boolean }`. `local_last_resort` is present and true only when an installed on-device model appended behind a remote-only chain actually served; an explicitly selected local model is not marked.
...
- The builtin identity `openai/gpt-5.6-sol:high` invokes one ephemeral, tool-free `codex exec` turn using Codex's ChatGPT subscription; pin that exact row with `model_id`. CAR removes `OPENAI_API_KEY` from the child and never reads the Codex token. It accepts one text prompt plus optional context and rejects tools, multi-turn messages, multimodal input, response schemas, and `infer_stream`. The `:high` model suffix maps to Codex `model_reasoning_effort="high"`. Codex emits no exact output-cap option, so `max_tokens` is an approximate final-answer instruction on this source; `usage` comes from Codex's `turn.completed` event and is never fabricated.
...
- **The armed deadline also OWNS the remote transport budget.** For
  non-streaming provider requests the engine derives its per-phase (request
  send, response body read) and retry budgets from the remaining deadline: a
  provider that stays silent for minutes (a reasoning model over a large
  prompt) is kept alive up to the caller's bound instead of being cut by
  internal transport ceilings, and a retry that cannot fit in what remains is
  not attempted. When the deadline is what terminates the call, the error is a
  deadline termination that NAMES the applied deadline ("deadline exceeded:
  the caller's 600000 ms infer deadline elapsed after …"), never a generic
  transient transport failure. Without an armed deadline, each transport phase
  is bounded at 300 s and transient failures retry up to 3 attempts with 3 s /
  6 s backoff, exactly as before.
...
- `ModelSource::CodexCli` is deliberately non-streaming: `codex exec --json` exposes completed answer items rather than token deltas. A pinned `openai/gpt-5.6-sol:high` streaming request returns an unsupported-mode error and must be retried through `infer`.
...
// client → server (typed terminal error response)
{ "jsonrpc": "2.0", "error": { "code": -32000, "message": "receipt journal unavailable", "data": { "terminal": true } }, "id": 17 }
```

The optional `error.data.terminal` boolean defaults to false. Only the NAPI or
PyO3 `TerminalToolError` (or an explicit raw-WS field) opts in; message text is
never classified.
